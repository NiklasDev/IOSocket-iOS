...
// Generators
+ (void)socketWithHost:(NSString *)hostURL params:(NSDictionary*)params response:(void (^)(IOSocket *))response {
...
+ (void)socketWithHost:(NSString *)hostURL params:(NSDictionary*)params reconnectAutomatically:(BOOL)reconnectAutomatically attemptLimit:(NSInteger)attempts withDelay:(NSTimeInterval)reconnectionDelay maximumDelay:(NSTimeInterval)maximumDelay timeout:(NSTimeInterval)timeout response:(void (^)(IOSocket *))response {
    return [self socketWithHost: hostURL
                         params: params
         reconnectAutomatically: YES
                   attemptLimit: -1
    ...
    for (id arg in args) {
        if ([arg isKindOfClass: [NSNull class]]) {
            ...
            if ([NSJSONSerialization isValidJSONObject:arg]) {
                ...
                NSLog(@"IOSocket serialization error at %@", NSStringFromSelector(_cmd));
            }
        }
    }
